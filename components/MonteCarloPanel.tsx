...
export function MonteCarloPanel({ rows, isValid }: MonteCarloPanelProps) {
  ...
  const rowSignature = useMemo(() => rows.map((row) => `${row.id}:${row.nominal}:${row.plusTolerance}:${row.minusTolerance}:${row.direction}`).join("|"), [rows]);
  ...
  function runSimulation() {
    ...
    setSeed(nextSeed);
    setResult(runMonteCarloSimulation(rows, sampleCount, nextSeed));
  }
